...
func (s *Server) handleMarkdown(w http.ResponseWriter, r *http.Request) {
	...
	markdown, err := pageToMarkdown(bCtx, req)
	if err != nil {
	...
	s.store.LogRequest("/markdown", req.URL, http.StatusOK, elapsed)

	w.Header().Set("X-Browser-Ms-Used", fmt.Sprint(elapsed))
	writeJSON(w, http.StatusOK, successResponse(markdown))
}

// pageToMarkdown renders a page via chromedp and converts its HTML to Markdown.
// Shared with the /json and /snapshot handlers.
func pageToMarkdown(ctx context.Context, req CommonParams) (string, error) {
	...
	return htmlToMarkdown(html, req.URL)
}
